...
- **Gantt Chart View**: Introduced a brand new Gantt chart view for visualizing records and timelines.
- **OpenRouter AI Provider**: Added OpenRouter as a new AI agent provider, allowing access to a wider range of models.
- **Stealth Browser Agent**: Integrated `playwright-extra` and stealth plugins to improve the browser agent's reliability and bot detection bypass.

### Changed

- **Multi-View Synchronization**: Streamlined and consolidated record handling logic across Spreadsheet, Calendar, Map, and Gantt views for more consistent state updates.
- **Chatbot UI Enhancements**: Added 'disabled' state for AI models and refined generating/thinking status messages for better clarity.
- **Calendar & Map Refinement**: Enhanced calendar date filtering logic and cleaned up UI layout by removing redundant dividers.

### Fixed

- **Calendar Event IDs**: Standardized event ID handling as strings to ensure consistency across different database providers.
